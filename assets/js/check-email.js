document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- CHECK EMAIL LOGIC ---
    // Check URL params for fallback reset link
    const urlParams = new URLSearchParams(window.location.search);
    const fallbackLink = urlParams.get('fallback');
    
    if (fallbackLink) {
        // Show debug section if there's a fallback link
        const debugSection = document.getElementById('debug-section');
        const resetLink = document.getElementById('reset-link');
        const resetLinkDisplay = document.getElementById('reset-link-display');
        
        if (debugSection && resetLink && resetLinkDisplay) {
            debugSection.classList.remove('hidden');
            resetLink.href = decodeURIComponent(fallbackLink);
            resetLink.textContent = 'Click here to reset password';
            resetLinkDisplay.classList.remove('hidden');
        }
    }
    
    // Test functionality for manual email lookup
    const getResetLinkBtn = document.getElementById('get-reset-link');
    if (getResetLinkBtn) {
        getResetLinkBtn.addEventListener('click', async () => {
            const email = prompt('Nhập email để lấy reset link:');
            if (!email) return;
            
            try {
                const response = await fetch(`/api/debug-reset-link/${email}`);
                const data = await response.json();
                
                const resetLink = document.getElementById('reset-link');
                const resetLinkDisplay = document.getElementById('reset-link-display');
                
                if (data.success && resetLink && resetLinkDisplay) {
                    resetLink.href = data.resetLink;
                    resetLink.textContent = 'Click here to reset password';
                    resetLinkDisplay.classList.remove('hidden');
                } else {
                    alert('❌ ' + (data.message || 'Không tìm thấy reset token'));
                }
            } catch (error) {
